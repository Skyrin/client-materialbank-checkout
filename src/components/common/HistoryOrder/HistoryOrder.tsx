...
import { HistoryOrderItem, Item } from "components/common/HistoryOrderItem/HistoryOrderItem";
...
export class HistoryOrder extends React.Component<Props, State> {
  ...
  render() {
    ...
    const statusClassName = "status_" + order.status.replace(" ", "-").toLowerCase();
    ...
    return (
      <div className={cn(styles["UserOrderHistory"])}>
        ...
        <div className={cn(styles["content"], styles["header"], styles["row"])}>
          <div className={cn(styles["left-container"], styles["container"], styles["placement-date"])}>
            <span>Order placed&nbsp;</span> {order.placeDate.monthLong} {order.placeDate.day}, {order.placeDate.year}
          </div>
          ...
          <div className={cn(styles["middle-container"], styles["container"])}>
            ...
            <div className={cn(styles["status-dot"], styles[statusClassName])}/>
          ...
          <div className={cn(styles["right-container"], styles["container"])}>
            ...
            <div className={cn(styles["light-text"], styles["order-number"])}>{order.orderNumber}</div>
          </div>
        </div>
        ...
        <div className={cn(styles["content"], styles["order-items"])}>
          {expanded ?
            order.items.map(item =>
              <HistoryOrderItem item={item} onClick={() => this.onItemClick(item)} key={item.id}/>
            )
            :
            order.items.slice(0, itemLimit).map(item =>
              <HistoryOrderItem item={item} onClick={() => this.onItemClick(item)} key={item.id}/>
            )
          }
        </div>
        ...
        <button className={cn(styles["content"], styles["expand-button"], expanded && styles["expanded"])}
                onClick={() => this.toggleExpand()}>
          {expanded ? "Hide full order" : "Show full order"} <i className="far fa-chevron-down"/>
